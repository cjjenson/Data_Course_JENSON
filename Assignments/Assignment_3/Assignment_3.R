...
#This plots are different because of the different factors involved
...
grep("41mm", dat$Headwidth )


# CONVERT THOSE TO "41.000"

dat$Headwidth=str_replace(dat$Headwidth,"41mm","41.000")


# DO THE SAME FOR "", BUT CONVERT THOSE TO "NA"
dat$Headwidth=plyr::mapvalues(dat$Headwidth, from = c(""), to = c("NA"))
...
col2 = c()
col3 = c()
df3=data.frame(Clothes=col1, Numbers= col2, Factor_numbers=col3)
...
#fill it in
col4=levels(dat$Size.class)
col5=new_vector
size_class_mean_mass = data.frame(Levels=col4, Mean= col5 )
...
ggplot(dat, aes(x=Headwidth..mm., y=Mass, col=Colony)) + geom_point()

# 2.  Write the code to save it (with meaningful labels) as a jpeg file

jpeg(filename = "Thatch_ANT_Headwidth_V_Mass.jpeg")
ggplot(dat, aes(x=Headwidth..mm., y=Mass, col=Colony)) + geom_point() + ggtitle("Headwidth V Mass") + xlab("Headwidth (mm)")
dev.off
...
write_csv(dat5,"../../Data_Course_Jenson/Assignments/Assignment_3/Dat5")
